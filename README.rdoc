...
* set_date, set_week, set_month helper method

  the three helper method in controller can easily help you to set @date.
...
You can configure the following default values by overriding these values using <tt>DatePaginate.configure</tt> method.
  num_pages           # 6 by default
  date_paginate_type  # :months by default (:days, :weeks, :months)
